...
```
console.log sends a message to the console, while return returns a value from a function
```

**_What's the difference between defining and calling a function?_**

```
Defining a function simply creates a function but calling is what utilizes it. It's like starting up a car(function definted). The car is ready to go, but you actually have to get inside and drive it(calling) to get use out of it.
```
...
```
A string representation of an object, usually denoted by quotations and usually consisting of words/letters, although numbers can be strings as well. (let x = "2")
```

**_Number_**

```
An integer. Must be a whole number, although negatives values are allowed.
```

**_Boolean_**

```
A data type who's value must be 'true' or 'false'. Case sensitive in JavaScript.
```

**_Array_**

```
A 'list' of values or variables, denoted by square brackets.
```
